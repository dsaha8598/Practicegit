 ...
 *
 */
public class DeducteeDeclarationExcel extends Excel<DeducteeDeclaration, DeclarationErrorDTO> {
	...
	public static List<FieldMapping> fieldMappings = Collections
			.unmodifiableList(Arrays.asList(new FieldMapping(HEADER_DEDUCTEE_CODE, "deducteeCode", "deducteeCode"),
					...
					new FieldMapping(HEADER_RATE_TYPE, "rateType", "rateType", Excel.VALIDATION_MANDATORY),
					new FieldMapping(HEADER_TDS_OR_TCS, "tdsOrTcs", "tdsOrTcs", Excel.VALIDATION_MANDATORY)));

	public DeducteeDeclarationExcel(XSSFWorkbook workbook) {
		super(workbook, fieldMappings, DeducteeDeclaration.class, DeclarationErrorDTO.class);
	}

	@Override
	public DeducteeDeclaration get(int index) {
		DeducteeDeclaration declaration = new DeducteeDeclaration();
