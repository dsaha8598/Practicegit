...
@Component({
  selector: 'ey-roles-permission-form',
  templateUrl: './roles-permission-form.component.html',
  styleUrls: ['./roles-permission-form.component.scss']
})
export class RolesPermissionFormComponent implements OnInit {
  ...
  getOnboardingParameters() {
    this.onboardingService.getConfigurations(this.deductorPan).subscribe(
      (res: any) => {
        console.log('Onboarding list and senind only ipp', res.data.ipp);
        this.checkPanLdcExistancy(res.data.ipp);
  ...
  removeOnboardingRolePermissions() {
    ...
    if (tabIndex != -1) {
      ...
      const rolepermissionGroupIndex = this.permissionsListData[
        ...
        if (x.authority !== null) {
          x.authority.permissionDisplayName == 'Roles and Permissions';
        }
      });
  ...
  checkPanLdcExistancy(res: any) {
    ...
    if (!isPanLdc) {
      ...
      const ldcIndex = this.permissionsListData[
        ...
        (x: any) => x.authority.permissionDisplayName == 'Validation Ldc'
      );
  ...
  checkInvoiceProcess(res: any) {
    ...
    const isSAP = res.includes(1);
    const isExcel = res.includes(2);
    const isPdf = res.includes(3);
    if (!isSAP) {
      const index = this.permissionsListData[tabIndex].groupNameList.findIndex(
        (x: any) => x.authority.permissionDisplayName == 'Invoice sap'
      );
    ...
    if (!isPdf) {
      const index = this.permissionsListData[tabIndex].groupNameList.findIndex(
        (x: any) => x.authority.permissionDisplayName == 'Invoice pdf'
      );
      this.permissionsListData[tabIndex].groupNameList.splice(index, 1);
    }
  }
  ...
  checkTransactionGLExistancy(res: any) {
    ...
    if (!isGL) {
      ...
      const glIndex = this.permissionsListData[
        tabIndex
      ].groupNameList.findIndex((x: any) => x.groupName == 'Transactions-gl');
  ...
  getPermssionsList() {
    this.rolePermissionService.getPermissions().subscribe(
      (res: any) => {
        console.log('Prinitng permission list', res);
  ...
  showRolePermissions(event: any) {
    this.showPermissions = true;
    const roleIndex = this.rolesListData.findIndex(
  ...
  initiatePermissions(res: any) {
    ...
    for (let i = 0; i < this.permissionsListData.length; i++) {
      for (let j = 0; j < res.length; j++) {
        /*   console.log("response of J",res[j]); */
        ...
        this.groupIndex = this.groupNames.findIndex((x: any) => {
          /*    console.log("xx",x);
          console.log("permission name",x.authority.permissionName)
       */ if (
            x.authority !== null
          ) {
            x.authority.permissionName == res[j];
          }
        });
        ...
        if (this.permissionsListData[i]['groupNameList'].length != 0) {
          for (let g = 0; g < this.groupNames.length; g++) {
            ...
            const labelIndex = this.labelNames.findIndex(
              (x: any) => x.authority.permissionName == res[j]
            );
            if (labelIndex != -1) {
              this.permissionsListData[i]['groupNameList'][g]['labels'][
                labelIndex
  ...
  onPermissionSelect(
    ...
    if (event.target.checked) {
      console.log('inside true if checked');
      this.authorizedPermissions.push(value);
      console.log(this.authorizedPermissions);
      debugger;
      this.selectedTabName = permission;
      console.log(this.selectedTabName);
    } else {
      console.log('inside true else checked');
      const index = this.authorizedPermissions.findIndex(
        (x: any) => x == value
      );
      if (index != -1) {
  ...
  checkViewPermission(event: any, tabIndex: any, groupIndex: any) {
    if (event.target.checked) {
      ...
      const labelIndex = labelsList.findIndex(
        (x: any) => x.authority.permissionDisplayName == 'List'
      );
  ...
  tabUncheck(checked: any, tabIndex: any, groupIndex: any): void {
    if (checked === false) {
      ...
      for (let i = 0; i < groupList.length; i++) {
        ...
        const groupIndex = this.authorizedPermissions.findIndex(
          (x: any) => x == groupList[i].authority.permissionName
        );
        ...
        for (let j = 0; j < lablesList.length; j++) {
          ...
          const labelIndex = this.authorizedPermissions.findIndex(
            (x: any) => x == lablesList[j].authority.permissionName
          );
  ...
  groupUncheck(checked: any, tabIndex: any, groupIndex: any): void {
    if (checked === false) {
      ...
      for (let i = 0; i < labelList.length; i++) {
        ...
        this.selectedLabels[labelName] = false;
        const labelIndex = this.authorizedPermissions.findIndex(
          (x: any) => x == labelList[i].authority.permissionName
        );
        this.authorizedPermissions.splice(labelIndex, 1);
      }
    }
