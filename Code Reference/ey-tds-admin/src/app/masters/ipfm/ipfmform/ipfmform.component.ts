...
@Component({
  selector: 'ey-ipfmform',
  templateUrl: './ipfmform.component.html',
  styleUrls: ['./ipfmform.component.scss']
})
export class IpfmformComponent implements OnInit {
  ...
  backFormClick(): void {
    this.router.navigate(['/dashboard/masters/Interest&penalty']);
  }
  reset(): void {
  ...
  fetchDatabyId(id: number): void {
    this.ipfmService.getIpfmById(id).subscribe(result => {
