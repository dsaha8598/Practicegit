...
export class SurchargeService {
  RESOURCE_URL = `${environment.api.masters}surcharge`;
  ...
  private readonly CATEGORY_URL = `${environment.api.masters}shareholderCategories`;
  private readonly SHAREHOLDERTYPE_URL = `${environment.api.masters}shareholderTypes`;
  ...
  getCategoryList(): Observable<any> {
    return this.http
      .get<any>(this.CATEGORY_URL)
      .pipe(map((res: any) => res.data));
  }

  getTypeOfShareholderList(): Observable<any> {
    return this.http
      .get<any>(this.SHAREHOLDERTYPE_URL)
      .pipe(map((res: any) => res.data));
  }
