...
@Component({
  selector: 'ey-natureofpaymentform',
  templateUrl: './natureofpaymentform.component.html',
  styleUrls: ['./natureofpaymentform.component.scss']
})
export class NatureofpaymentformComponent implements OnInit {
  submittedTo: boolean;
  dateError: boolean;
  ...
  initialLoading(): void {
    this.paymentForm = new FormGroup({
      ...
      considerDateofPayment: new FormControl(false),
  ...
  saveNatureOfPayment(type: any): void {
    this.submitted = true;
    ...
    this.natureofpaymentService
      .addNatureOfPayment(this.paymentForm.getRawValue() as INatureOfPayment)
      .subscribe(
  ...
  fetchDatabyId(params: any): void {
    this.natureofpaymentService.getNatureOfPaymentById(params.id).subscribe(
      (result: any) => {
  ...
  backFormClick(): void {
    this.router
      .navigate(['/dashboard/masters/natureofpayment'])
