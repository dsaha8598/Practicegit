...
@Component({
  selector: 'ey-cesstypeTcsform',
  templateUrl: './cesstype-tcsform.component.html',
  styleUrls: ['./cesstype-tcsform.component.scss']
})
export class CesstypeTcsformComponent implements OnInit {
  ...
  constructor(
    ...
    private readonly cesstypeTcsService: CesstypeTcsService,
  ...
  fetchDatabyId(id: number): void {
    this.cesstypeTcsService.getCessDatabyId(id).subscribe(
      (result: ICessType) => {
  ...
  goBack(): void {
    this.router.navigate(['/dashboard/masters/cesstypeTcs']);
  }
  get f(): any {
