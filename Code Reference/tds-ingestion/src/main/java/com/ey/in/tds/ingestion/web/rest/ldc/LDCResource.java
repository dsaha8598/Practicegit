package com.ey.in.tds.ingestion.web.rest.ldc;
...
public class LDCResource {
	 ...
	 * @throws ParseException
	 */
	@GetMapping("ldc/matrix/{year}/{certificateNumber}")
	public ResponseEntity<ApiStatus<Map<Integer, Object>>> getLDCMatrix(
		...
		Map<Integer, Object> ldc = ldcUtilizationService.getLDCMatrix(deductorTan, year, certificateNumber);

		if (logger.isInfoEnabled()) {
			logger.info("LDC all matrix values is done ");
		}
		ApiStatus<Map<Integer, Object>> apiStatus = new ApiStatus<Map<Integer, Object>>(HttpStatus.OK, "SUCCESS",
				"LIST OF LDC MATRIX DATA", ldc);
		return new ResponseEntity<ApiStatus<Map<Integer, Object>>>(apiStatus, HttpStatus.OK);
	}

	/**
	 * This method downloads file for LDC adjusted amount
	 * 
	 * @param tan
	 * @param tenantId
	 * @param userName
	 ...
	 * @throws Exception
	 */
	@PostMapping(value = "/ldc/matrix/adjusted/file/download")
	public ResponseEntity<ApiStatus<String>> ldcMatrixAdjustedFileDownload(
		...
		ldcUtilizationService.asyncLdcMatrixAdjustedFileDownload(tan, year, month, cerificateNumber, tenantId,
				userName);
		String message = "Ldc adjusted report requested successfully and will be available shortly";
		logger.info("{}", message);
	 ...
	 * @throws Exception
	 */
	@PostMapping(value = "/ldc/matrix/closingAmount/file/download")
	public ResponseEntity<ApiStatus<String>> ldcMatrixClosingAmountDownload(
			@RequestHeader(value = "TAN-NUMBER") String tan, @RequestHeader("X-TENANT-ID") String tenantId,
			@RequestHeader(value = "USER_NAME") String userName, @RequestBody Map<String, String> ldcData)
			throws Exception {
		...
		ldcUtilizationService.asyncLdcMatrixClosingAmountDownload(tan, year, month, cerificateNumber, type, tenantId,
				userName);
		String fileType = "";
		...
		String message = "Ldc " + fileType + " report requested successfully and will be available shortly";
		logger.info("{}", message);
		ApiStatus<String> apiStatus = new ApiStatus<>(HttpStatus.OK, "SUCCESS", message, message);
		return new ResponseEntity<>(apiStatus, HttpStatus.OK);
	}
	...
	/**
	 * to download the LDC utilization report using ldc id
	 * 
	 * @param deductorTan
	 * @param lccMasterId
	 * @param tenantId
	 ...
	 * @throws Exception
	 */
	@PostMapping("ldc-utilization/export/{id}")
	public ResponseEntity<ApiStatus<String>> tcsLccUtilizationFileExport(
		...
		ldcUtilizationService.generateLdcUtilizationFileAsync(lccMasterId, tenantId, deductorTan, userName, pan);
